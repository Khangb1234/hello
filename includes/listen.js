...
(async function () {
  ...
  const rainbow1 = chalk1.rainbow(`[ INFO BOT ] ${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "This bot was made by CatalizCS and SpermLord" : global.config.BOTNAME}`).stop();
rainbow1.render();
  ...
  const rainbow = chalk1.rainbow(`[ TIME ] ==${gio}==`).stop();
rainbow.render();
